...
export class BusinessConfigService {
  ...
  private applyTheme(config: BusinessConfig): void {
    ...
    const tokens: Record<string, string | undefined> = {
      ...
      '--color-cta-hover': theme?.cta ? `color-mix(in oklch, ${theme.cta}, black 10%)` : undefined,
      '--color-bg': theme?.background ?? config.brand.bgBase,
      '--color-surface': theme?.surface,
      '--color-surface-2': theme?.surfaceAlt,
      '--color-surface-3': theme?.surfaceAlt ? `color-mix(in oklch, ${theme.surfaceAlt}, ${theme.mode === 'dark' ? 'white' : 'black'} 8%)` : undefined,
      '--color-border': theme?.border,
      '--color-divider': theme?.border,
      '--color-text': theme?.text,
      '--color-text-muted': theme?.textMuted,
      '--color-text-faint': theme?.textMuted ? `color-mix(in oklch, ${theme.textMuted}, ${theme.mode === 'dark' ? 'white' : 'black'} 12%)` : undefined,
